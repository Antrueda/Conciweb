...
//Funcion para cargar combos y vista del formulario
    ...
    public function modalMensajeBienvenida()
    {
        ...
        // $horaCierre = Carbon::createFromFormat('H:i:s', '17:00:00'); // 
        // $horaAbrir = Carbon::createFromFormat('H:i:s', '08:00:00'); // 
        $estadoform =Estadoform::first();
        
        // $horaActual = Carbon::now();
        // // dd($horatext->greaterThanOrEqualTo($horaAbrir)&&$horatext->lessThanOrEqualTo($horaCierre));
        // if ($horaActual->greaterThanOrEqualTo($horaCierre)&&$horaActual->lessThanOrEqualTo($horaAbrir)) {
        //     // La hora actual ha pasado o es igual a la hora establecida
        //     $estadoform->update(['sis_esta_id' =>3 ]);
        // } else {
        //     if ($horaActual->greaterThanOrEqualTo($horaAbrir)&&$horaActual->lessThanOrEqualTo($horaCierre)) {
        //     // La hora actual es anterior a la hora establecida
        //     $estadoform->update(['sis_esta_id' =>1 ]);
        //     }
        // }


        switch ($estadoform->sis_esta_id) {
            case 1:
...
//Funcion de consulta de solicitud
    public function autosearch(Request $request)
    {
        ...
        // $tramite= tramiteusuario::whereDate('fec_solicitud_tramite', '<=', now()->subWeekdays($diasParam))
        // ->where('estado_tramite', 'Remitido')
        // ->where('vigencia', Carbon::today()->isoFormat('YYYY'))
        // ->where('id_tramite', 335)->each(function ($tramite) use ($diasParam) {

        //     $tramite->realizarCambioDespuesDe5Dias($diasParam);
        // });
        // // $respues= tramiterespuesta::whereDate('FEC_RESPUESTA', '<=', now()->subWeekdays($diasParam))
        // // ->where('ESTADO_TRAMITE', 'Remitido')
        // // ->where('vigencia', Carbon::today()->isoFormat('YYYY'))
        // // ->where('id_tramite', 335)->each(function ($respues) use ($diasParam) {
...
//Funcion para el cambio de estado de la conciliacion
    ...
    {  
        ...
        $dato = ModelsTramiteusuario::where('num_solicitud', $consulta->num_solicitud)->where('vigencia', $consulta->vigencia)->first();
        ...
        $detalleAbc = Subdescripcion::where('subasu_id', $dato->subasunto)
            ...
            $conteo= count($detalleAbc)-1;
            try {
                $subject = 'Desistimiento de Conciliación Web No. '.$consulta->num_solicitud;
                $data = array(
                    'email' => $dato->email,
                    'asuntos' => $dato->asuntos->nombre,
                    'nombrecompleto' => $nombrecompleto,
