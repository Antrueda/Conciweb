...
<form name="frmRegistroDatos" enctype="multipart/form-data" id="frmRegistroDatos">
    @foreach ($data['estadoTipoAudi'] as $info)
    <input type="hidden" name="estadoTipoAudi" id="estadoTipoAudi" value="{!! $info->estado !!}">
    @endforeach
    ...
    <div class="card">
        ...
        <div class="card-body">
            <div class="row">
                <div class="col-md-6">
                    <label class="form-group has-float-label">
                        <select class="form-control form-control-sm custom-select validate[required]" name="tipoDocumento" id="tipoDocumento">
            ...
            <div class="row">
                ...
                <div class="col-md-3">
                    <label class="form-group has-float-label">
                        <select class="form-control form-control-sm custom-select validate[optional]" name="localidad" id="localidad">
                            <option value=" ">- Seleccione una opcion -</option>
                            @foreach ($data['listaLocalidades'] as $info)
                            <option value="{{$info->codloc}}">{!! $info->descloc !!}</option>
                            @endforeach
                        </select>
        ...
        <div class="card">
            ...
            <div class="card-body">
                <div class="row">
                    <div class="col-md-6">
                        <label class="form-group has-float-label">
                            <select class="form-control form-control-sm custom-select validate[required]" name="tipoDocApoderado" id="tipoDocApoderado">
                                <option value=" ">- Seleccione una opcion -</option>
                                @foreach ($data['listaTipoDoc'] as $info)
                                <option value="{{$info->sicidtipodocumentoidentidad}}">{!! $info->sictipodocumentoidentidad !!}</option>
                                @endforeach
            ...
            <div class="card-body">
                ...
                <div id="tipoAudienciaSeleccion" style="display: none">
                    ...
                    <div class="alert alert-success" role="alert">
                        @foreach ($data['mensaje'] as $info)
                        <small class="text-justify">{!! $mensaje = $info->descripcion !!}</small>
                        @endforeach
                    </div>
                    <div class="row">
                        <div class="col-4">
                        ...
                        <div id="sedesConciliacion" style="display: none">
                            <div class="row">
                                <div class="col-6">
                                    <label class="form-group has-float-label">
                                        <select class="form-control form-control-sm custom-select validate[required]" name="sedePrincipal" id="sedePrincipal">
                                            <option value=" ">- Seleccione una opcion -</option>
                                            @foreach ($data['listaSedes'] as $info)
                                            <option value="{!! $info->idsede !!}">{!! $info->nombresede !!}</option>
                                            @endforeach
                                        </select>
                                        <span>13.1. Centro de conciliación principal *</span>
                                    </label>
                                </div>
                                <div class="col-6">
                                    <label class="form-group has-float-label">
                                        <select class="form-control form-control-sm custom-select validate[required]" name="sedeSecundaria" id="sedeSecundaria">
                                            <option value=" ">- Seleccione una opcion -</option>
                                            @foreach ($data['listaSedes'] as $info)
                                            <option value="{!! $info->idsede !!}">{!! $info->nombresede !!}</option>
                                            @endforeach
                                        </select>
                                        <span>13.2. Centro de conciliación secundaria *</span>
                                    </label>
                                </div>
                            </div>
                        </div>
                ...
                <div id="datosConciliacion" style="display: none">
                    <div class="row">
                        <div class="col-md-12">
                            <label class="form-group has-float-label">
                                <select class="form-control form-control-sm custom-select validate[required]" name="asunto" id="asunto">
                                    <option value=" ">- Seleccione una opcion -</option>
                                    @foreach ($data['listaAsuntos'] as $info)
                                    <option value="{{$info->id}}">{{$info->descripcion}}</option>
                                    @endforeach
                                </select>
                                <span>14. Asunto *</span>
                            </label>
                        ...
                        <div class="col-md-12">
                            <label class="form-group has-float-label">
                                <div class="input-group input-file" name="document1">
                                    ...
                                    <input type="text" class="validate[required] form-control" name="document1" id="document1" placeholder=' 15) Adjuntar Soporte *' />
                                    <span class="input-group-btn">
                                        <button class="btn btn-danger btn-reset" type="button">Limpiar</button>
                                    </span>
                                </div>
                                <span style="margin-top: 11px;"> 17. Adjuntar Soporte (Máximo 10Mb) *</span>
                            </label>
                        </div>
                    </div>
                </div>
...
<script>
    ...
    $("#tipoSolicitud").change(function() {
        ...
        if (tipoSolicitud == 0) {
            //Solicitud - Directa
            $("#askAudienciaPresencial").show(); //Pregunta Audiencia Presencial
            $("#seccionApoderado").hide(); //Ocultar seccion para msotrr los datos del apodedaro
        } else if (tipoSolicitud == 1) {
            //Solicitud - Apoderado
            $("#seccionApoderado").show(); //Mostrar seccion para msotrr los datos del apodedaro
            $("#askAudienciaPresencial").show(); //Pregunta Audiencia Presencial
        } else {
    ...
    $("#asunto").one("click dblclick change", () => {
        $.ajax({
            ...
            success: function(opciones) {
                ...
                opciones.forEach(element => $('#subAsunto').append(new Option(element['descripcion'], element['id'])));
            }
        })
    });

    //Extraer infromacion de documentos solicitados
    $("#subAsunto").one("click dblclick change", () => {
        $.ajax({
